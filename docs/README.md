...
| [草案-DSH本地用量-技术方案.md](草案-DSH本地用量-技术方案.md) | **未实施**：`UsageApp` 中没有 dsh。接入 DSH(含 DSH Desktop)本地会话日志作为用量数据源,归并为单一「DSH」条目。核心是复刻官方帧扫描算法 + Apple Compression 逐帧解压(不引第三方依赖),并按现有四扫描器契约做帧边界对齐的增量 watermark |
